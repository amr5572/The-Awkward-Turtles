...
var vel=400,rock,boarder,trap,grass,enemy,enemyGroup,scoreText,lifeText,wolfGroup,skelGroup;
...
hero=new Hero(game,99*32,133*32);
...
map.createFromObjects('wolfEnemy',12,'',0,true,false,wolfGroup,Wolf);
map.createFromObjects('skeletonEnemy',13,'',0,true,false,skelGroup,Skeleton);
...
// setup player health display //
    heroHealthBar=new HealthBar(this.game,{bar:{color:'#42f450'}});
    heroHealthBar.setPosition(220,80);
    heroHealthBar.setFixedToCamera(true);
...
///bug testing bounderis
    game.debug.body(wolfGroup);
}
,
hitEnemy: function(s,e){
e.damage(1); if(e==boss){bossHealthBar.setPercent(boss.health);}
    
if(e.health<=0){bossSound.stop('boss_sound'); mage.kill(); mage.alive=false; score+=1; bossHealthBar.kill(); bossText = game.add.text(104*32, 157.4*32, "Cleared!", { font: '70px Impact', fill: '#000' });
}
...
function hitbyEnemy(h,e){
  ...
  h.alpha=0.2;
  h.damage(5);
  heroHealthBar.setPercent(h.health/80 * 100);
